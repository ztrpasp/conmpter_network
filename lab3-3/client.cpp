...
int N;                   // min(cwnd,windowsize)
unsigned int status = 0; // RENO算法状态机
...
DWORD WINAPI clientSend(LPVOID lparam)
{
    ...
    while (true)
    {
        ...
        if (isStop == true)
            return 0;
        if (clock() - start > MAX_TIMEOUT || isDupThree)
        {

            if (isDupThree && (status == 0 || status == 1))
            {
                ssthresh = cwnd / 2;
                cwnd = ssthresh + 3;
                status == 2;//进入快速恢复阶段
                isDupThree = false;
                cout << "重复接收！重传" << base << "到" << nextseqnum - 1 << "号数据" << endl;
            }
            else 
            {
                ssthresh = cwnd / 2;
        ...
        else
        {
            while (nextseqnum <= packetNum)
            {
                ...
                if (nextseqnum < base + N)
                {
                    ...
                    nextseqnum++;
                    printf("cwnd: %.3f  ssthresh: %d  ",cwnd,ssthresh);
                    cout << "base:  " << base << "nextseqnum:   " << nextseqnum << "end:    " << base + N << endl;
                    // start = clock();
                }
                else
                    break;
    ...
    while (true)
    {
        if (recvfrom(p->clientSocket, pktBuffer, sizeof(Packet), 0, (SOCKADDR *)&p->serverAddr, &addrLen) > 0)
        {
            ...
            if (base > rcvPkt.head.ack || CalcheckSum((u_short *)&rcvPkt, sizeof(Packet)) != 0) // 忽略相同的ACK
            {
                ...
                dupACKCount++;
                if(dupACKCount==3) isDupThree = true;
				if (status == 2) {//快速恢复
					cwnd += 1;
				}
            }
            else
            {
                if (status == 0)
				{
					cwnd += rcvPkt.head.ack - base + 1;//累积确认
					if (cwnd >= ssthresh)
						status = 1;//拥塞避免
				...
				{
					cwnd += (float)((rcvPkt.head.ack - base + 1) / cwnd);//线性增长
				}
                else if (status == 2)
				{
					cwnd = ssthresh;
